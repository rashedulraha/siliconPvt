...
import {
  FacebookIcon, InstagramIcon, LinkedinIcon, YoutubeIcon, TwitterIcon,
  Mail, Phone, MapPin, ChevronRight, Send,
  Shield, Award, Users, TrendingUp,
  Building2, FileText, Briefcase, Camera, BookOpen,
  Link2,
} from "lucide-react";
...
export function Footer() {
  ...
  const handleNewsletter = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setEmail(""); setEmailError(""); setSubmitting(false);
  };
  ...
  const socialLinks = [
    ...
    { icon: InstagramIcon, key: "instagram", url: siteSettings.social.instagram },
      ...
      <div className="border-b border-white/8">
        <Container className="py-10">
          <div className="flex items-center gap-3 mb-7">
            ...
            <h3 className="text-label text-white/50">Footer Sitemap — All Links</h3>
          </div>
            ...
            {sitemapColumns.map((col) => (
              <div key={col.title} className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-label text-white/45 text-[10px]">{col.title}</h4>
                </div>
          ...
          <div className="mt-7 pt-5 border-t border-white/8 flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-1.5">
              {[
                ...
                <Link key={label} href={href} className="text-xs text-white/35 hover:text-accent transition-colors">
                  {label}
                </Link>
              ))}
            </div>
      ...
      <Container className="py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-4 space-y-5">
            <Link href="/" className="flex items-center gap-3 w-fit group">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-accent/20 border border-accent/30">
                <span className="font-heading font-bold text-accent text-lg leading-none">S</span>
              </div>
              <div>
                <span className="font-heading font-semibold text-white text-lg tracking-tight block leading-tight">
                  {siteSettings.siteName}
                </span>
                <span className="text-white/35 text-[10px] tracking-widest uppercase">Real Estate</span>
              </div>
            </Link>

            <p className="text-white/50 text-sm leading-[1.8] font-light max-w-xs">
              A trusted name in land development across Dhaka. RAJUK-approved plots backed
              by legal verification and full transparency.
            </p>
          ...
          <div className="lg:col-span-2 space-y-4">
            <h3 className="text-label text-white/45 text-[11px]">Quick Links</h3>
            <ul className="space-y-2.5">
              {[
                ...
                <li key={link.label}>
                  <Link href={link.href} className="flex items-center gap-1.5 text-sm text-white/45 hover:text-accent transition-colors group">
          ...
          <div className="lg:col-span-4 space-y-5">
            <div>
              <h3 className="text-label text-white/45 text-[11px] mb-2">Stay Updated</h3>
              <p className="text-sm text-white/40 font-light leading-relaxed">
                Subscribe for the latest property listings, investment tips and updates.
              </p>
            </div>
        ...
        <div className="mt-10 pt-6 border-t border-white/8 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-white/30">
          <p>© {new Date().getFullYear()} {siteSettings.siteName} (Pvt.) Ltd. All Rights Reserved.</p>
          <p>
