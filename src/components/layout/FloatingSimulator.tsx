...
export function FloatingSimulator() {
  ...
  return (
    <div
      className={cn(
        "fixed bottom-6 left-6 z-50 flex flex-col gap-2 rounded-2xl bg-card/90 border border-border/80 p-3 shadow-soft-lg backdrop-blur-md select-none transition-all duration-300 max-w-[260px]",
      ...
      {isExpanded && (
        <div className="pt-2 mt-2 border-t border-border/60 space-y-2 animate-in fade-in slide-in-from-bottom-2 duration-200">
          ...
          <div className="bg-muted/50 rounded-xl px-2.5 py-1.5 border border-border/40 text-[10px] text-muted-foreground">
            <span className="font-medium text-foreground block">Session State:</span>
            {isLoggedIn && user ? (
              <span className="mt-0.5 block truncate text-primary font-semibold">
                {user.role === "admin" ? "👑 Admin" : "👤 User"}: {user.name}
              </span>
            ) : (
              <span className="mt-0.5 block text-muted-foreground">🌐 Guest / Public Visitor</span>
            )}
          </div>
