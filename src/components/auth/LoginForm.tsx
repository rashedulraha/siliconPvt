...
export default function LoginForm() {
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			login(mockUserData);
			toast.success(
				`Authenticated securely as ${role === "admin" ? "Administrator" : "Client"}`,
			);
			router.push(role === "admin" ? "/dashboard/admin" : "/dashboard/user");
		} catch (err) {
			setError(
				"An anomaly occurred during the authentication routing pipeline.",
			);
		...
		return (
			<div className="flex flex-col items-center justify-center p-8 space-y-4">
				...
				<p className="text-sm text-muted-foreground font-light">
					Verifying credentials integrity...
				</p>
			</div>
		);
	}
	...
	if (isLoggedIn) {
		return (
			<div className="flex flex-col items-center justify-center p-8 space-y-4">
				<Loader2 className="w-6 h-6 animate-spin text-primary" />
				<p className="text-sm text-muted-foreground font-light">
					Redirecting to session environment...
				</p>
			</div>
		);
	}
		...
		<div className="w-full space-y-6">
			<FadeInSlideUp className="space-y-2 text-center lg:text-left">
				...
				<p className="text-xs text-neutral-500 dark:text-neutral-400 font-light leading-relaxed">
					Access your verified assets portfolio dashboard using secure
					...
					<div className="flex items-center justify-between">
						...
						<a
							...
							onClick={(e) => {
								e.preventDefault();
								toast.info(
									"Password resetting via secure registered emails is under testing.",
								);
							}}
