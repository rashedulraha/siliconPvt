...
import { 
  Building2, Users, DollarSign, TrendingUp, Mail, Phone, MapPin, 
  Plus, Edit3, Trash2, Save, LogOut, CheckCircle2, ChevronRight,
  Sliders, Settings, Globe, Eye, FileText, LayoutGrid, CheckCircle
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "leads" | "properties" | "cms">("overview");
  ...
  const [leads, setLeads] = useState<MockLead[]>([
    { id: "lead-1", name: "Tanvir Ahmed", phone: "01712345678", email: "tanvir@gmail.com", property: "Silicon Orchard - Plot 5", status: "agreement", value: 4800000 },
    { id: "lead-2", name: "Farhana Yasmin", phone: "01819234567", email: "farhana.y@outlook.com", property: "Silicon Royal Heights - Apt 3C", status: "site_visit", value: 13500000 },
    { id: "lead-3", name: "Imran Khan", phone: "01911987654", email: "imran.khan@dhaka.io", property: "Silicon Commercial Square - Plot 12", status: "new", value: 21000000 },
    { id: "lead-4", name: "Ziaur Rahman", phone: "01511223344", email: "zia@yahoo.com", property: "Silicon Green Valley - Plot 22", status: "contacted", value: 3200000 },
    { id: "lead-5", name: "Sadia Chowdhury", phone: "01612344321", email: "sadia.c@gmail.com", property: "Silicon Vista - Penthouse 9B", status: "won", value: 42000000 }
  ]);
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden max-w-8xl mx-auto w-full">
        ...
        <aside className="w-full lg:w-60 bg-card border-r border-border/50 p-4 space-y-2 flex-shrink-0">
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg transition-all ${
              activeTab === "overview" ? "bg-primary/8 text-primary shadow-xs" : "text-muted-foreground hover:bg-muted hover:text-foreground"
            }`}
          >
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg transition-all ${
              activeTab === "leads" ? "bg-primary/8 text-primary shadow-xs" : "text-muted-foreground hover:bg-muted hover:text-foreground"
            }`}
          >
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg transition-all ${
              activeTab === "properties" ? "bg-primary/8 text-primary shadow-xs" : "text-muted-foreground hover:bg-muted hover:text-foreground"
            }`}
          >
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg transition-all ${
              activeTab === "cms" ? "bg-primary/8 text-primary shadow-xs" : "text-muted-foreground hover:bg-muted hover:text-foreground"
            }`}
          >
          ...
          <div className="border-t border-border/60 my-4 pt-4">
            <Link href="/" target="_blank" className="w-full flex items-center gap-3 px-3 py-2 text-xs font-semibold rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 bg-background">
          ...
          {activeTab === "properties" && (
            <div className="space-y-6">
              <div>
                <h2 className="text-xl font-heading font-bold text-foreground tracking-tight">Land &amp; Flat Inventory</h2>
                <p className="text-xs text-muted-foreground mt-0.5">Add, remove, or view current plots and flat listings in Dhaka.</p>
              </div>
              ...
              <Card className="border-border/50 shadow-xs rounded-2xl bg-card overflow-hidden">
                ...
                <CardContent className="pt-5">
                  <form onSubmit={handleAddProperty} className="grid sm:grid-cols-3 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase">Property Title</label>
                      <input 
                        type="text" 
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase">Price (BDT)</label>
                      <input 
                        type="number" 
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase">General Location</label>
                      <input 
                        type="text" 
                        placeholder="e.g. Purbachal, Dhaka" 
                        value={newProp.location} 
                        onChange={(e) => setNewProp(p => ({ ...p, location: e.target.value }))}
                        className="w-full text-xs border border-border rounded-lg px-3 py-2 bg-background focus:outline-none focus:border-primary/50"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase">Detailed Address</label>
                      <input 
                        type="text" 
                    ...
                    <div className="space-y-1">
                      ...
                      <select 
                        ...
                        onChange={(e) => setNewProp(p => ({ ...p, category: e.target.value }))}
                        className="w-full text-xs border border-border rounded-lg px-3 py-2 bg-background focus:outline-none focus:border-primary/50"
                      >
                        <option value="land">Residential/Commercial Plot (Land)</option>
                        <option value="apartment">Ready Flat (Apartment)</option>
                      </select>
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase">Area (sqft or decimals)</label>
                      <input 
                        type="number" 
                    ...
                    <div className="space-y-1 sm:col-span-3">
                      ...
                      <input 
                        ...
                        onChange={(e) => setNewProp(p => ({ ...p, features: e.target.value }))}
                        className="w-full text-xs border border-border rounded-lg px-3 py-2 bg-background focus:outline-none focus:border-primary/50"
                      />
                    </div>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      ...
                      <tbody>
                        {state.properties.map((prop) => (
                          <tr key={prop.id} className="border-b border-border/30 last:border-0 hover:bg-muted/10 transition-colors">
                            <td className="p-4 font-semibold text-foreground">{prop.title}</td>
                            <td className="p-4 text-muted-foreground">{prop.location}</td>
                            <td className="p-4 font-mono font-medium text-foreground">৳ {prop.price.toLocaleString("en-IN")}</td>
          ...
          {activeTab === "cms" && (
            <div className="space-y-6">
              <div>
                <h2 className="text-xl font-heading font-bold text-foreground tracking-tight">Visual CMS Editor</h2>
                ...
                <CardHeader className="pb-3 border-b border-border/40 flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-xs">
                      Update values displayed in header, footer, and contact sections.
                    </CardDescription>
                  </div>
                  <Button size="sm" onClick={handleSaveCMS} className="bg-primary hover:bg-primary/95 text-white">
                ...
                <CardContent className="pt-6 space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase">Company Site Name</label>
                      <input 
                        type="text" 
                        value={siteName} 
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase">Contact Hotline Phone</label>
                      <input 
                        type="text" 
                        value={phoneVal} 
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase">Business Operating Hours</label>
                      <input 
                        type="text" 
                        value={hoursVal} 
                    ...
                    <div className="space-y-1.5 sm:col-span-2">
                      <label className="text-[10px] font-bold text-muted-foreground uppercase">Corporate HQ Address</label>
                      <input 
                        type="text" 
                        value={addrVal} 
              ...
              <Card className="border-border/50 shadow-xs rounded-2xl bg-card">
                ...
                <CardContent className="pt-4 space-y-2 text-xs">
                  <p className="text-muted-foreground">Click below to navigate directly to the respective pages and view your CMS updates:</p>
                  <div className="flex flex-wrap gap-2 pt-2">
                    <Link href="/" target="_blank" className="border border-border px-3 py-1.5 rounded-lg bg-background hover:bg-muted flex items-center gap-1.5 font-medium">
                      <Globe className="w-3.5 h-3.5 text-primary" /> Homepage
                    </Link>
                    <Link href="/about" target="_blank" className="border border-border px-3 py-1.5 rounded-lg bg-background hover:bg-muted flex items-center gap-1.5 font-medium">
                      <FileText className="w-3.5 h-3.5 text-primary" /> About Us
                    </Link>
                    <Link href="/projects" target="_blank" className="border border-border px-3 py-1.5 rounded-lg bg-background hover:bg-muted flex items-center gap-1.5 font-medium">
                      <Sliders className="w-3.5 h-3.5 text-primary" /> Projects
                    </Link>
                    <Link href="/properties" target="_blank" className="border border-border px-3 py-1.5 rounded-lg bg-background hover:bg-muted flex items-center gap-1.5 font-medium">
                      <Building2 className="w-3.5 h-3.5 text-primary" /> Properties
                    </Link>
                  </div>
                </CardContent>
              </Card>
