      ...
      <Container className="py-8 max-w-7xl space-y-8">
        ...
        <StaggerContainer className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <StaggerItem>
              <PremiumHoverCard glowColor="rgba(245, 158, 11, 0.15)" className="relative overflow-hidden rounded-2xl bg-neutral-900 dark:bg-[#070D1E] text-white p-6 sm:p-8 border border-neutral-800 dark:border-neutral-800/45 shadow-xl">
                    ...
                    {PIPELINE_STEPS.map((step, idx) => {
                      ...
                      return (
                        <div key={step.key} className="relative group/step">
                          ...
                          >
                            {isDone && <Check className="w-2.5 h-2.5 text-emerald-500 stroke-[3]" />}
                ...
                <CardContent className="pt-4 px-4 space-y-4">
                  {savedPropertiesList.length > 0 ? (
                    savedPropertiesList.map((prop) => (
                      <div key={prop.id} className="flex gap-3 pb-3 border-b border-neutral-100 dark:border-neutral-950/20 last:border-0 last:pb-0 items-start group/wishlist">
                    ...
                    <div className="text-center py-6 px-4 flex flex-col items-center justify-center space-y-4">
                      ...
                      <div className="space-y-1 max-w-[220px]">
                        <h5 className="text-xs font-semibold text-neutral-700 dark:text-neutral-300">Wishlist is empty</h5>
                        <p className="text-[10px] text-neutral-400 dark:text-neutral-500 font-light leading-relaxed">
                          Save luxury developments to track their pricing, title registry, and construction schedules.
                        </p>
                      </div>
                      <Button asChild size="sm" variant="outline" className="text-[11px] h-8 px-4 border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-900 rounded-xl transition-all cursor-pointer">
                        <Link href="/properties" className="flex items-center gap-1">
                          Explore Collection <ArrowRight className="w-3 h-3 ml-0.5" />
                        </Link>
                      </Button>
                    </div>
                  )}
              ...
              <PremiumHoverCard className="border border-neutral-200/60 dark:border-neutral-800/60 shadow-xs rounded-2xl bg-white dark:bg-[#111E35] p-3 md:p-5">
                ...
                <CardContent className="pt-4 px-4 space-y-4 pb-6">
                  {recommendedProperties.map((prop) => (
                    <div key={prop.id} className="flex gap-3 pb-3 border-b border-neutral-100 dark:border-neutral-950/20 last:border-0 last:pb-0 items-start group/recom">
                      <div className="relative w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 bg-muted border border-border/50">
                        <Image
                          src={prop.images[0] || "https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=150"}
                          alt={prop.title}
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        <Link href={`/properties/${prop.slug}`} className="font-semibold text-xs text-neutral-800 dark:text-neutral-200 hover:text-primary transition-colors block truncate flex items-center justify-between gap-1">
