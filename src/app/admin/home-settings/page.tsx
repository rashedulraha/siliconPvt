...
export default function HomeSettingsPage() {
	...
	const addTrustCounter = () => {
		setFormData((prev) => ({
			...
			trustCounters: [
				...prev.trustCounters,
				{ value: "New Stat", label: "Description Label", detail: "Verified Detail" },
			],
		}));
	};
	...
	const handleSave = async (e?: React.FormEvent) => {
		...
		try {
			...
			setSavedMessage("Home page content updated and saved to database successfully!");
					...
					<div className="flex items-center justify-between gap-4">
						...
						<div className="flex items-center gap-2.5">
							...
							<button
								onClick={handleSave}
								disabled={saving}
								className="px-5 h-9 rounded-xl bg-primary text-primary-foreground text-xs font-bold font-heading inline-flex items-center gap-2 transition-all shadow-xs hover:bg-primary/90 cursor-pointer disabled:opacity-50"
							>
								{saving ? (
				...
				<div className="max-w-5xl mx-auto space-y-6">
					...
					<div className="space-y-1 text-left">
						...
						<p className="text-xs sm:text-sm text-muted-foreground font-light">
							Control all dynamic texts, counters, badges, and banners displayed on the public website Home landing page.
						</p>
					</div>
						...
						{tabs.map((tab) => {
							...
							return (
								<button
									...
									className={`p-3 rounded-2xl text-left flex flex-col justify-between transition-all cursor-pointer border ${
										isActive
						...
						<Card className="border border-border/70 rounded-2xl shadow-xs overflow-hidden">
							<CardHeader className="bg-muted/20 border-b border-border/40 pb-4">
								...
								<CardDescription className="text-xs text-muted-foreground font-light">
									Configure the primary title and subtitle that appear over the interactive 3D hero carousel and featured plot listings.
								</CardDescription>
							</CardHeader>
					...
					{activeTab === "masterplan" && (
						<Card className="border border-border/70 rounded-2xl shadow-xs overflow-hidden">
							<CardHeader className="bg-muted/20 border-b border-border/40 pb-4">
								...
								<CardDescription className="text-xs text-muted-foreground font-light">
									Configure the two dual highlight cards that introduce the master layout and civic facilities.
								</CardDescription>
							</CardHeader>
								...
								<div className="space-y-4 p-4 rounded-2xl bg-card border border-border/60">
									...
									<div className="grid sm:grid-cols-2 gap-4">
										<div className="space-y-2">
											...
											<Input
												...
												onChange={(e) => handleChange("masterPlanBadge", e.target.value)}
										...
										<div className="space-y-2">
											...
											<Input
												...
												onChange={(e) => handleChange("masterPlanTitle", e.target.value)}
									...
									<div className="space-y-2">
										...
										<Textarea
											...
											onChange={(e) => handleChange("masterPlanDesc", e.target.value)}
									...
									<div className="space-y-2">
										...
										<Input
											...
											onChange={(e) => handleChange("masterPlanImage", e.target.value)}
									...
									<div className="grid sm:grid-cols-2 gap-4">
										<div className="space-y-2">
											...
											<Input
												...
												onChange={(e) => handleChange("amenitiesBadge", e.target.value)}
										...
										<div className="space-y-2">
											...
											<Input
												...
												onChange={(e) => handleChange("amenitiesTitle", e.target.value)}
					...
					{activeTab === "showcase" && (
						<Card className="border border-border/70 rounded-2xl shadow-xs overflow-hidden">
							<CardHeader className="bg-muted/20 border-b border-border/40 pb-4">
								...
								<CardDescription className="text-xs text-muted-foreground font-light">
									Update the mega township presentation section title and geographic summary.
								</CardDescription>
							</CardHeader>
							<CardContent className="p-6 space-y-5">
								<div className="space-y-2">
									...
									<Input
										...
										onChange={(e) => handleChange("showcaseBadge", e.target.value)}
								...
								<div className="space-y-2">
									...
									<Textarea
										...
										onChange={(e) => handleChange("showcaseDesc", e.target.value)}
							...
							<CardHeader className="bg-muted/20 border-b border-border/40 pb-4">
								<div className="flex items-center justify-between">
									<div>
										...
										<CardDescription className="text-xs text-muted-foreground font-light">
											Numerical stats and milestones displayed inside the dark track record container.
										</CardDescription>
									</div>
									<Button
										type="button"
								...
								<div className="grid sm:grid-cols-2 gap-4">
									<div className="space-y-2">
										...
										<Input
											...
											onChange={(e) => handleChange("trackRecordTitle", e.target.value)}
									...
									<div className="space-y-2">
										...
										<Input
											...
											onChange={(e) => handleChange("trackRecordDesc", e.target.value)}
												...
												<div className="space-y-2">
													<div>
														...
														<Input
															value={counter.value}
															onChange={(e) =>
																handleTrustCounterChange(idx, "value", e.target.value)
															}
													...
													<div>
														...
														<Input
															value={counter.label}
															onChange={(e) =>
																handleTrustCounterChange(idx, "label", e.target.value)
															}
													...
													<div>
														...
														<Input
															...
															onChange={(e) =>
																handleTrustCounterChange(idx, "detail", e.target.value)
															}
							...
							<CardHeader className="bg-muted/20 border-b border-border/40 pb-4">
								<div className="flex items-center justify-between">
									<div>
										...
										<CardDescription className="text-xs text-muted-foreground font-light">
											Badges and legal certifications displayed in the compliance strip on the Home landing page.
										</CardDescription>
									</div>
									<Button
										type="button"
								...
								<div className="space-y-3">
									{formData.accreditations.map((badge, idx) => (
										<div key={idx} className="flex items-center gap-2.5">
											...
											<Input
												value={badge}
												onChange={(e) => handleAccreditationChange(idx, e.target.value)}
					...
					{activeTab === "cta" && (
						<Card className="border border-border/70 rounded-2xl shadow-xs overflow-hidden">
							<CardHeader className="bg-muted/20 border-b border-border/40 pb-4">
								...
								<CardDescription className="text-xs text-muted-foreground font-light">
									Configure the bottom high-converting CTA banner with transport scheduling and office registration details.
								</CardDescription>
							</CardHeader>
							<CardContent className="p-6 space-y-5">
								<div className="space-y-2">
					...
					<div className="pt-4 flex justify-end">
						<button
							onClick={handleSave}
							disabled={saving}
							className="px-6 h-11 rounded-2xl bg-primary text-primary-foreground text-xs font-bold font-heading inline-flex items-center gap-2 transition-all shadow-xs hover:bg-primary/90 cursor-pointer disabled:opacity-50"
						>
							{saving ? (
