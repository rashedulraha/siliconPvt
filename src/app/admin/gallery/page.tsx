...
export default function AdminGalleryPage() {
	...
	const fetchItems = async () => {
		setLoading(true);
		try {
			const res = await apiFetch<{ success: boolean; items?: GalleryItem[] }>(
				"/gallery",
			);
	...
	const handleConfirm = async () => {
		...
		try {
			if (confirmAction.type === "delete") {
				await apiFetch(`/gallery/${confirmAction.item.id}`, {
					method: "DELETE",
				});
				...
				if (confirmAction.item) {
					const res = await apiFetch<{ success: boolean; item?: GalleryItem }>(
						`/gallery/${confirmAction.item.id}`,
						{
							method: "PUT",
				...
				<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
					<div>
						...
						<p className="text-xs text-muted-foreground font-light mt-0.5">
							{items.length} items in gallery • Manage project photos,
							infrastructure, and handover albums
						</p>
					</div>
				...
				{!isEditorUnlocked && (
					<div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 text-xs font-medium flex items-center justify-between gap-4">
						<div className="flex items-center gap-2">
							...
							<span>
								<strong>Read-Only Mode:</strong> Unlock Editor Mode to add,
				...
				{loading ? (
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
						{Array.from({ length: 6 }).map((_, i) => (
							<div
								key={i}
								className="rounded-3xl bg-muted/60 animate-pulse aspect-[4/3]"
							/>
						))}
					</div>
				) : items.length === 0 ? (
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
						{items.map((item) => (
							<div
								key={item.id}
								className="group bg-card border border-border/70 rounded-[24px] overflow-hidden flex flex-col hover:border-border transition-all"
							>
								{/* Cover Image */}
								<div className="relative aspect-[16/10] bg-muted overflow-hidden">
									...
									<div className="absolute top-3 right-3 flex gap-1.5">
										<span
											className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${item.active ? "bg-emerald-500/20 text-emerald-500 border border-emerald-500/20" : "bg-muted text-muted-foreground border border-border/40"}`}
										>
								...
								<div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
									<div className="space-y-1">
										...
										<h3 className="text-sm font-bold font-heading text-foreground line-clamp-2">
											{item.title}
										</h3>
						...
						>
							...
							<div className="flex items-center justify-end gap-3 pt-1">
								...
								>
									...
									{confirmAction.type === "delete"
										? "Yes, Delete"
										: `Yes, ${confirmAction.item ? "Update" : "Create"}`}
								</button>
							</div>
						</motion.div>
					</motion.div>
