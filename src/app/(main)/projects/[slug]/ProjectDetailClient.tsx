			...
			.filter(
				(p) =>
					...
					(p.category === project.category ||
						p.location.includes(project.location.split(",")[0])),
			)
			.slice(0, 3);
		...
		return (
			<SectionContainer className="py-32 text-center min-h-[60vh] flex flex-col items-center justify-center">
				...
				<p className="text-zinc-500 dark:text-zinc-400 mb-8 max-w-md font-light">
					The project you are looking for does not exist, has been archived, or
					has a different URL path.
				</p>
				<Button
					asChild
					className="rounded-full bg-[#0D1B3E] dark:bg-white text-white dark:text-[#0D1B3E] hover:bg-[#0D1B3E]/90 dark:hover:bg-white/90"
				>
					<Link href="/projects">
			...
			<div className="pt-32 border-b border-zinc-200/50 dark:border-white/10 bg-zinc-50/20 dark:bg-[#0D1B3E]/40">
				<SectionContainer className="py-4 flex flex-wrap items-center justify-between gap-4">
					...
					<nav className="flex items-center gap-2 text-xs text-zinc-400 dark:text-zinc-500">
						...
						<ChevronRight className="h-3 w-3" />
						<span className="text-zinc-800 dark:text-white font-medium truncate max-w-[200px]">
							{project.title}
						</span>
					</nav>
				</SectionContainer>
					...
					<div className="space-y-10">
						...
						<div className="space-y-4">
							<div className="flex flex-wrap gap-2.5 items-center">
								<span
									className={`inline-flex items-center px-3.5 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase border ${STATUS_STYLES[project.status] || ""}`}
								>
							...
							<div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
								...
								<div className="w-full md:w-auto md:text-right bg-zinc-50 dark:bg-[#0D1B3E]/40 border border-zinc-200/50 dark:border-white/10 rounded-2xl p-4 min-w-[200px] flex-shrink-0">
									<span className="text-[9px] text-zinc-400 dark:text-zinc-500 uppercase tracking-widest font-bold block mb-1">
										Starting Price
									</span>
						...
						<div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-6 rounded-[24px] border border-zinc-200/50 dark:border-white/10 bg-white/60 dark:bg-[#0D1B3E]/30 backdrop-blur-md">
							<div className="space-y-1">
								<span className="text-[9px] text-zinc-400 dark:text-zinc-500 uppercase tracking-widest font-bold">
									Total Area
								</span>
							...
							<div className="space-y-1">
								...
								<div className="flex items-center gap-2">
									...
									<span className="text-sm font-semibold text-zinc-850 dark:text-white capitalize">
										{project.category}
									</span>
								</div>
							</div>
						...
						{project.description && (
							<div className="space-y-4">
								<h2 className="font-heading font-semibold text-xl text-zinc-950 dark:text-white flex items-center gap-2">
									<Sparkles className="h-5 w-5 text-[#D4A030]" /> About This
									Project
								</h2>
						...
						{project.features.length > 0 && (
							<div className="space-y-4">
								<h2 className="font-heading font-semibold text-xl text-zinc-950 dark:text-white flex items-center gap-2">
									<Building2 className="h-5 w-5 text-[#D4A030]" /> Project
									Amenities & Features
								</h2>
								<div className="p-6 sm:p-8 rounded-[28px] border border-zinc-200/50 dark:border-white/10 bg-white/60 dark:bg-[#0D1B3E]/30 backdrop-blur-md">
										...
										{project.features.map((feature, idx) => (
											<div key={idx} className="flex items-start gap-3">
												<div className="h-5 w-5 rounded-full bg-[#D4A030]/10 flex items-center justify-center flex-shrink-0 mt-0.5">
													<span className="text-[#D4A030] text-[10px] font-black">
														✓
													</span>
												</div>
					...
					<aside className="space-y-6 lg:sticky lg:top-24">
						...
						<div className="bg-white/60 dark:bg-[#0D1B3E]/30 backdrop-blur-md border border-zinc-200/50 dark:border-white/10 rounded-[28px] p-6 shadow-sm space-y-4">
							<h3 className="font-heading font-semibold text-lg text-zinc-950 dark:text-white flex items-center gap-2">
								<CalendarCheck className="h-5 w-5 text-[#D4A030]" /> Schedule
								Site Visit
							</h3>
							<p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed font-light">
								Experience our layout plan first-hand. Our advisors are
								available all week to show you the plots and ready blocks.
							</p>
							<Button
								asChild
								className="w-full h-11 rounded-xl font-semibold shadow-sm bg-[#0D1B3E] dark:bg-white hover:bg-[#0D1B3E]/90 dark:hover:bg-white/90 text-white dark:text-[#0D1B3E]"
							>
						...
						{emiDetails && (
							<div className="bg-white/60 dark:bg-[#0D1B3E]/30 backdrop-blur-md border border-zinc-200/50 dark:border-white/10 rounded-[28px] p-6 shadow-sm space-y-5">
								<div className="flex items-center gap-2">
									...
									<h3 className="font-heading font-semibold text-lg text-zinc-950 dark:text-white">
										EMI Estimator
									</h3>
								</div>
									...
									<div className="space-y-2">
										<div className="flex justify-between text-xs font-light">
											...
											<span className="font-medium text-zinc-800 dark:text-white">
												{downPaymentPercent}% (
												{formatCompactCurrency(emiDetails.downPayment)})
											</span>
										</div>
										<input
											type="range"
								...
								<div className="space-y-2 pt-1">
									{agent.phone && (
										<a
											...
											className="flex items-center gap-2.5 text-xs text-zinc-700 dark:text-zinc-300 hover:text-[#D4A030] dark:hover:text-[#D4A030] transition-colors py-2 px-3 rounded-xl bg-zinc-50/50 dark:bg-white/5 border border-zinc-200/40 dark:border-white/10 font-light"
										>
											<Phone className="h-3.5 w-3.5 text-[#D4A030]" />{" "}
											{agent.phone}
										</a>
									)}
									{agent.email && (
										<a
											...
											className="flex items-center gap-2.5 text-xs text-zinc-700 dark:text-zinc-300 hover:text-[#D4A030] dark:hover:text-[#D4A030] transition-colors py-2 px-3 rounded-xl bg-zinc-50/50 dark:bg-white/5 border border-zinc-200/40 dark:border-white/10 font-light truncate"
										>
											<Mail className="h-3.5 w-3.5 text-[#D4A030]" />{" "}
											{agent.email}
										</a>
									)}
								</div>
						...
						<div className="bg-white/60 dark:bg-[#0D1B3E]/30 backdrop-blur-md border border-zinc-200/50 dark:border-white/10 rounded-[28px] p-6 shadow-sm">
							<h3 className="font-heading font-semibold text-lg text-zinc-950 dark:text-white flex items-center gap-2 mb-2">
								<MessageSquare className="h-5 w-5 text-[#D4A030]" /> Send
								Enquiry
							</h3>
								...
								{formSubmitted ? (
									...
									>
										...
										<Button
											...
											className="w-full h-10 rounded-xl font-semibold bg-[#0D1B3E] dark:bg-white text-white dark:text-[#0D1B3E] hover:bg-[#0D1B3E]/90 dark:hover:bg-white/90"
										>
				...
				{relatedProjects.length > 0 && (
					<div className="pt-20 border-t border-zinc-200/50 dark:border-white/10 mt-16 space-y-8">
						<div className="flex items-center justify-between">
							...
							<Link
								href="/projects"
								className="text-xs font-semibold text-[#D4A030] hover:text-[#D4A030]/90 transition-colors flex items-center gap-1"
							>
						...
						<div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
							{relatedProjects.map((property) => (
								<div
									key={property.id}
									className="group flex flex-col h-full rounded-[28px] border border-zinc-200/50 dark:border-white/5 bg-white/60 dark:bg-[#0D1B3E]/30 backdrop-blur-md overflow-hidden hover:border-[#D4A030]/40 hover:shadow-xl transition-all duration-300"
								>
									...
									<div className="p-6 flex flex-col justify-between flex-grow gap-5">
										...
										<div className="flex items-center justify-between pt-3 border-t border-zinc-100 dark:border-white/5">
											...
											<Link
												...
												className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 hover:text-[#D4A030] dark:hover:text-[#D4A030] transition-colors"
											>
												Details →
											</Link>
										</div>
