...
const MOCK_GALLERY: GalleryItem[] = [
  {
    ...
    description: "High-definition drone mapping of residential sectors, internal roadways leveling, and land boundaries demarcation.",
  ...
  },
  {
    ...
    description: "Cinematic aerial survey tracking progress on structural concrete pouring and core column reinforcements.",
  ...
  },
  {
    ...
    description: "Drone review showing the leveling of internal layout pathways and connection works with the bypass highway.",
  ...
  },
  {
    ...
    description: "Fully ready plots showing finished boundary brickwork, electric installation meters, and internal avenue saplings.",
  ...
  },
  {
    ...
    description: "Modern commercial building facade displaying insulated glass fixtures, entrance lobbies, and perimeter landscaping.",
  ...
  },
  {
    ...
    description: "Pristine, Apple Minimalist client lounge at our headquarters, featuring micro-cement textures and premium oak accents.",
  ...
  },
  {
    ...
    description: "Final site delivery detailing custom structural concrete features, exterior water elements, and automated safety gates.",
  ...
  },
  {
    ...
    description: "Approved engineering layout blueprint outlining zoning splits, residential sectors, internal paths, and utility networks.",
  ...
  },
  {
    ...
    description: "Detailed foundation civil engineering plan mapping structural load bearings, shear wall calculations, and basement pillars.",
    url: "https://images.unsplash.com/photo-1581094288338-2314dddb7eed?w=1600&q=80",
    thumbnail: "https://images.unsplash.com/photo-1581094288338-2314dddb7eed?w=800&q=80",
  ...
  },
  {
    ...
    description: "Approved foundation concrete casting grid details highlighting primary columns and lift shaft elevator grids.",
...
export default function GalleryPage() {
  ...
  const cmsItems: GalleryItem[] = (state.media || []).map((media) => {
    ...
    if (media.type.includes("video") || media.url.endsWith(".mp4") || media.url.endsWith(".webm")) {
      category = "drone";
    } else if (
    ...
    return {
      ...
      date: new Date(media.uploadedAt).toLocaleDateString("en-US", { month: "short", year: "numeric" }),
    };
  });
  ...
  const openLightbox = (item: GalleryItem) => {
    ...
    setInquiryData({ name: "", phone: "", message: `Interested in project details for: ${item.title}` });
  };

  const prevItem = () => {
    if (filteredItems.length === 0) return;
    const idx = (lightboxIndex - 1 + filteredItems.length) % filteredItems.length;
  ...
  const handleInquirySubmit = async (e: React.FormEvent) => {
    ...
    toast.success(`Inquiry sent for ${lightbox.title}. An advisor will contact you shortly.`);
  };
          ...
          <div className="flex items-start justify-between flex-wrap gap-6">
            <div>
              ...
              <p className="text-white/55 text-lg font-light max-w-xl">
                Explore real estate development drone flyovers, project milestones, and project blueprints.
              </p>
            </div>
          </div>
        </div>
        ...
        <AnimatePresence mode="popLayout">
          {filteredItems.length === 0 ? (
            ...
            >
              ...
              <h3 className="font-semibold text-lg text-foreground">No visual assets found</h3>
              <p className="text-muted-foreground text-sm mt-1 max-w-xs mx-auto">
                No matching results were found. Try resetting filters or search criteria.
              </p>
              <Button
                onClick={() => {
                  setActiveCat("all");
                  ...
                  <div className="relative aspect-video w-full overflow-hidden bg-muted border-b border-border">
                    ...
                    <div className="absolute top-4 left-4 z-10">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider bg-background/95 border border-border text-foreground shadow-xs">
                        {item.category === "drone" && <Play className="w-2 h-2 fill-current text-primary" />}
                        {item.category === "progress" && <Camera className="w-2.5 h-2.5" />}
                        {item.category === "blueprint" && <FileText className="w-2.5 h-2.5" />}
                ...
                <div className="mt-8 pt-6 border-t border-border">
                  {inquirySuccess ? (
                    ...
                    >
                      ...
                      <h4 className="font-semibold text-foreground text-xs">Consultation Requested</h4>
                      <p className="text-muted-foreground text-[11px] leading-normal">
                        Our advisor for {lightbox.location} will contact you shortly.
                      </p>
                    </motion.div>
                  ) : (
                    <div className="space-y-3">
                      <h4 className="text-xs font-semibold text-foreground">Request Asset Consultation</h4>
